...
(async () => {
  ...
  await put('students/av1', { id: 'av1', firstName: 'Avans', lastName: 'Sinov', status: 'faol', phone: '+998900000001' });
  await put('groups/avg1', { id: 'avg1', code: 'AV01', name: 'Sinov guruh', status: 'faol', fee: 500000, days: [1], startTime: '09:00', endTime: '10:00' });
  await put('invoices/avi1', { id: 'avi1', studentId: 'av1', groupId: 'avg1', month: '2026-09', final: 500000, dueDate: '2026-09-05' });
  await put('invoices/avi2', { id: 'avi2', studentId: 'av1', groupId: 'avg1', month: '2026-10', final: 500000, dueDate: '2026-10-05' });
  ...
  const p1 = await req('/api/payment', {
    method: 'POST', cookie,
    body: {
      id: 'av_pay1', studentId: 'av1', amount: 800000, date: '2026-09-02', method: 'naqd',
      allocations: [{ invoiceId: 'avi1', amount: 500000 }]
    }
  });
  ...
  const adv = await req('/api/payment', {
    method: 'POST', cookie,
    body: {
      id: 'av_adv1', type: 'advance', studentId: 'av1', amount: 300000, date: '2026-10-01',
      allocations: [{ invoiceId: 'avi2', amount: 300000 }]
    }
  });
  ...
  const invoices = Object.values(all.json.col.invoices).filter(i => i.studentId === 'av1');

  const received = payments.filter(p => p.studentId === 'av1' && !p.voided)
    .reduce((s, p) => s + (p.type === 'refund' ? -1 : 1) * p.amount, 0);
  const allocated = payments.filter(p => p.studentId === 'av1' && !p.voided)
  ...
  const over = await req('/api/payment', {
    method: 'POST', cookie,
    body: {
      id: 'av_adv2', type: 'advance', studentId: 'av1', amount: 200000, date: '2026-10-02',
      allocations: [{ invoiceId: 'avi2', amount: 200000 }]
    }
  });
  ...
  await put('students/av2', { id: 'av2', firstName: 'Ikki', lastName: 'Sinov', status: 'faol' });
  await put('invoices/avi3', { id: 'avi3', studentId: 'av2', groupId: 'avg1', month: '2026-09', final: 400000, dueDate: '2026-09-05' });
  await req('/api/payment', {
    method: 'POST', cookie,
    body: { id: 'av_pay3', studentId: 'av1', amount: 600000, date: '2026-10-03', method: 'naqd', allocations: [] }
  });
  const wrong = await req('/api/payment', {
    method: 'POST', cookie,
    body: {
      id: 'av_adv3', type: 'advance', studentId: 'av1', amount: 100000, date: '2026-10-03',
      allocations: [{ invoiceId: 'avi3', amount: 100000 }]
    }
  });
  ...
  eq('O’sha yozuv qaytdi', again.json.payment.id, 'av_adv1');
  const all2 = await req('/api/bootstrap', { cookie });
  const advCount = Object.values(all2.json.col.payments).filter(p => p.type === 'advance').length;
  ...
  const adv4 = await req('/api/payment', {
    method: 'POST', cookie,
    body: {
      id: 'av_adv4', type: 'advance', studentId: 'av1', amount: 500000, date: '2026-10-04',
      allocations: [{ invoiceId: 'avi2', amount: 500000 }]
    }
  });
  ...
  const pays3 = Object.values(all3.json.col.payments).filter(p => p.studentId === 'av1' && !p.voided);
